...
# Patch 4: Fix g permutation (verified as no-op, but keep for consistency)
RUN sed -i 's|ggml_permute(ctx0, g, 2, 0, 3, 1)|ggml_permute(ctx0, g, 1, 0, 2, 3)|' src/llama-build-context.cpp

# Patch 5: Debug value dumps in fused kernel (head 0, token 0, first 2 calls)
COPY debug_kernel_values.py /tmp/debug_kernel_values.py
RUN python3 /tmp/debug_kernel_values.py

# Verify patches
RUN echo '=== Patch 5 (debug vals) ===' && grep -n 'DN_VAL' ggml/src/ggml.c | head -5
