...
- Official GGUF: `Qwen/Qwen3-Coder-Next-GGUF` Q4_K_M (48.4GB, 4-part split) at `/mnt/Aether/appdata/models/qwen3-coder-next-q4km/`
- Ollama GGUF: Q4_K (~41GB) at `/mnt/Aether/appdata/ollama/models/blobs/sha256-30e51a7cb...` (used for ik fork testing only)
**Infrastructure:** Docker on TrueNAS/HexOS, BIOS UMA 16GB, Vulkan via `/dev/dri` passthrough
...
**Hardware:** Radeon 890M iGPU (16 RDNA 3.5 CUs), BIOS UMA 16GB, 96GB DDR5-5600 dual-channel

| Config | PP tok/s | TG tok/s | Quality |
|--------|----------|----------|---------|
| 30/49 layers on GPU (2GB VRAM BIOS, --no-mmap) | ~37 | 10.5 | Coherent 500+ tokens |
| **49/49 layers on GPU (16GB UMA, mmap)** | **57.0** | **15.3** | **Coherent 500+ tokens** |
| **49/49 layers + flash attention (`-fa on`)** | **48-54** | **15.0-17.6** | **Coherent 500+ tokens** |
| Federico's (Vulkan, Q8_0, Debian native) | N/A | ~11 | Coherent with 16K context |

#### Flash Attention on Vulkan RDNA 3.5
...
**FA verdict:** Slight TG boost for short responses, slight PP regression. Primary benefit is halved KV cache memory, enabling 8K-16K context without memory pressure. Recommended for coding tasks with moderate context.

#### Critical UMA Findings

1. **`--no-mmap` kills UMA performance.** Forces double allocation from shared DDR5 pool — model exists in both CPU RAM and GPU-mapped memory simultaneously. Caused `ErrorOutOfDeviceMemory` at 35+ GPU layers even with 16GB UMA + 39GB GTT. Dropping `--no-mmap` let all 49 layers load cleanly.

2. **BIOS UMA carveout reduces GTT.** Linux amdgpu computes `GTT ≈ (total RAM / 2) - VRAM`. Increasing UMA from 2GB to 16GB shrank GTT from ~46GB to ~39GB. Total Vulkan-visible stays ~55GB, but the flexible GTT pool (where tensor uploads actually go) is smaller. Don't over-allocate UMA — 16GB is sufficient for full offload.

3. **Docker passes `/dev/dri` through directly**, no restriction on GPU memory. The bottleneck is BIOS VRAM allocation, not Docker.
...
| `llama-vulkan-mainline` | Mainline llama.cpp + Vulkan iGPU | **PRODUCTION** — 15.3 tok/s, all 49 layers on GPU |

## Production Configuration

**Current best setup** (mainline llama.cpp + Vulkan, standard attention):

```bash
docker run -d --name llama-vulkan-prod \
  -p 8091:8080 \
  --device /dev/dri/renderD128:/dev/dri/renderD128 \
  --device /dev/dri/card0:/dev/dri/card0 \
  -v /mnt/Aether/appdata/models/qwen3-coder-next-q4km:/models:ro \
  llama-vulkan-mainline \
  -m /models/Qwen3-Coder-Next-Q4_K_M-00001-of-00004.gguf \
  -t 12 -c 4096 --host 0.0.0.0 --port 8080 -ngl 99 -fa on
```

**Key flags:**
- `-ngl 99` — all 49 layers on Vulkan iGPU
- `-fa on` — flash attention (halves KV cache, slight TG boost)
- NO `--no-mmap` — mmap is essential on UMA architecture
- `-c 4096` — context size (can push to 8K-16K with FA enabled)

**BIOS requirements:**
- UMA Frame buffer: 16GB (`Advanced → AMD CBS → NBIO Common Options → GFX Configuration`)
